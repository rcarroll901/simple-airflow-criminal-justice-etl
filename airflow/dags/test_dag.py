...
class TestDagStructure(unittest.TestCase):
    def setUp(self):
        ...
        self.current_scale_var = int(Variable.get('num_odyssey_scraping_tasks', default_var=1))

    def test_num_tasks(self):

        expected = 3 + self.current_scale_var # 3 single tasks and dynamic scaled task depending on var
    ...
    def test_dependencies_of_check_profiles_task(self):
        ...
        downstream_task_ids = list(map(lambda task: task.task_id, check_profiles_task.downstream_list))
        downstream_task_ids.sort()

        self.assertListEqual(downstream_task_ids,
                             [f'odyssey_scraper_{i}'
                                    for i
                                    in range(self.current_scale_var)])

        upstream_task_ids = list(map(lambda task: task.task_id, check_profiles_task.upstream_list))
